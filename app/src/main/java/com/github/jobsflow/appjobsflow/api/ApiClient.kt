...
sealed class ApiException(val json: Map<String, Any?>, message: String) : Exception(message) {
    ...
    companion object {
        // Вспомогательный метод: проверяет, является ли JSON-ответ ошибкой превышения лимита
        // Метод вызывается только если HTTP-ответ неуспешный (response.isSuccessful == false)
...
class ApiClient(
    ...
    fun getAuthorizeUrl(redirectUri: String = defaultRedirectUri, state: String = "", scope: String = "applicant"): String {
        val effectiveRedirect = if (redirectUri.isNotEmpty()) redirectUri else "hhandroid://oauthresponse"
        val params = mapOf(
            "client_id" to clientId,
            "redirect_uri" to effectiveRedirect,
    ...
    suspend fun authenticate(code: String): ApiClient {
        val effectiveRedirect = if (defaultRedirectUri.isNotEmpty()) defaultRedirectUri else "hhandroid://oauthresponse"
        val params = mapOf(
            "client_id" to clientId,
    ...
    protected fun defaultHeaders(): Headers {
        ...
        return Headers.Builder().apply {
            ...
            add("x-hh-app-active", "true")
        }.build()
    }

    protected fun throwApiException(response: Response, json: Map<String, Any?>) {
        // Мы попадаем сюда только потому, что response.isSuccessful == false.
        // Сначала проверяем JSON на признак LimitExceededException,
        // так как это наиболее специфичный случай ошибки, возможный с разными HTTP-кодами.
        if (ApiException.isLimitExceeded(json)) {
            throw LimitExceededException(json.toMap())
        }

        // Если это не LimitExceeded по JSON — проверяем по HTTP-коду.
        when (response.code) {
            ...
            429 -> throw TooManyRequestsException(json.toMap()) // Для общих 429
    ...
    protected fun handleToken(token: Map<String, Any?>) {
        ...
        val expiresInSec = ((token["expires_in"] as? Number)?.toLong() ?: 0)
        accessExpiresAt = expiresInSec * 1000 + System.currentTimeMillis()
        sharedPrefs?.let {
            with(it.edit()) {
                putString("token_type", token["token_type"] as? String)
                putString("scope", token["scope"] as? String)
                putLong("expires_in", expiresInSec)
